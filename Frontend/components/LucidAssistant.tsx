...
import { X, Send, HelpCircle, FileText, ClipboardList, Upload } from "lucide-react";
...
export default function LucidAssistant() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("openLucidAssistant", handleOpenAssistant);
    return () => {
      window.removeEventListener("openLucidAssistant", handleOpenAssistant);
    };
  }, []);
  ...
  // Only render the chat panel modal when open
  if (!open) return null;
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '12px 14px', overflow: 'hidden', flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {/* Menu shown when no mode selected and no messages */}
