...
export interface ReportItem {
  id: string;
  title: string;
  // Task-specific fields (for task_submissions mapping)
  taskId?: string;
  taskTitle?: string;
  employeeName?: string;
  ...
  // Optional fields for task_submissions fetched from Supabase
...
interface TaskReportsProps {
  ...
  isLoading?: boolean;
  defaultActiveSubTab?: 'assessment' | 'roleplay' | 'tasks';
...
export default function TaskReports({ reportsList = INITIAL_REPORTS, onAddSimulatedReport, isLoading = false, defaultActiveSubTab = 'assessment', employeeId = null }: TaskReportsProps) {
  ...
  const [fetchedReports, setFetchedReports] = useState<ReportItem[] | null>(null);
  const [fetching, setFetching] = useState(false);
  const [summaries, setSummaries] = useState<any[]>([]);

  useEffect(() => {
    // If employeeId is provided, fetch task_submissions for this user
    if (!employeeId) return;
    let cancelled = false;
    const load = async () => {
      setFetching(true);
      try {
        // Use backend API which returns joined submission, task, and user info
        ...
        if (!cancelled) {
          ...
          const mapped: ReportItem[] = rows.map((r: any) => {
              const taskTitle = (r.tasks && (Array.isArray(r.tasks) ? r.tasks[0]?.title : r.tasks.title)) || r.title || 'Untitled Task';
              const aiValidationObj = r.ai_validation || {
              ...
              const candidateFields = [
                ...
                taskObj?.task_type,
                taskObj?.kind,
                taskObj?.category_label,
                taskObj?.meta && taskObj.meta.category,
                taskObj?.meta && taskObj.meta.type
              ].filter(Boolean);

              let derivedCategoryRaw = candidateFields.length ? candidateFields[0] : null;

              // If no explicit field, try tags or title heuristics
              if (!derivedCategoryRaw && taskObj) {
                if (Array.isArray(taskObj.tags) && taskObj.tags.length) {
                  derivedCategoryRaw = taskObj.tags.join(' ');
                } else if (taskObj.title) {
                  // Use title as a last-resort hint
                  derivedCategoryRaw = taskObj.title;
                }
              }

              // If submission looks like an assessment (answers array), prefer assessment
              const looksLikeAssessment = Array.isArray(r.answers) && r.answers.length > 0;
              // If submission looks like a conversation/roleplay (messages, turns, conversation), prefer roleplay
              const looksLikeConversation = (
                (Array.isArray(r.messages) && r.messages.length > 0) ||
                (Array.isArray(r.conversation) && r.conversation.length > 0) ||
                (typeof r.message_count === 'number' && r.message_count > 0) ||
                (typeof r.turns === 'number' && r.turns > 0) ||
                Boolean(r.messages_count) ||
                Boolean(r.conversation_length)
              );

              let derivedCategory: 'assessment' | 'roleplay' | 'tasks' = 'tasks';
              if (looksLikeAssessment) {
                derivedCategory = 'assessment';
              } else if (looksLikeConversation) {
                derivedCategory = 'roleplay';
              } else if (derivedCategoryRaw) {
                const c = String(derivedCategoryRaw).toLowerCase();
                if (/role|roleplay|scenario|script|dialogue|conversation/.test(c)) derivedCategory = 'roleplay';
                else if (/assess|quiz|exam|test|mcq|question|answers|evaluation|score/.test(c)) derivedCategory = 'assessment';
                else if (/task|checklist|maintenance|sanit|verification|log|inspection|daily|bi-weekly|front-of-house/.test(c)) derivedCategory = 'tasks';
                else derivedCategory = 'tasks';
              }
              ...
              taskId: r.task_id,
              taskTitle: taskTitle,
              employeeName: (r.users && (r.users.name || r.users.email)) || 'Unknown User',
              submittedAt: r.submitted_at || null,
              response: r.text_response || null,
                category: derivedCategory,
              score: typeof r.score === 'number' ? r.score : (r.answers ? (Array.isArray(r.answers) ? r.answers.length : 0) : 0),
              totalQuestions: typeof r.max_score === 'number' ? r.max_score : 0,
              ...
              aiValidation: aiValidationObj,
              questionsList: Array.isArray(r.answers) ? r.answers.map((a: any, i: number) => ({
                question: a.question || `Item ${i+1}`,
                submittedAnswer: a.answer || a.submitted || JSON.stringify(a),
                correctAnswer: a.correct || '—',
                isCorrect: !!a.correct,
                points: typeof a.points === 'number' ? a.points : (a.correct ? 1 : 0)
              })) : undefined,
            } as ReportItem);
          }).map((obj: any) => ({ ...obj, assignment_id: rows.find((r: any) => r.submission_id === obj.id)?.assignment_id }));
  ...
  // When fetchedReports change, fetch summaries for related assignment_ids
  useEffect(() => {
    if (!fetchedReports || fetchedReports.length === 0) return;
    const assignmentIds = Array.from(new Set(fetchedReports.map((r: any) => (r as any).assignment_id).filter(Boolean)));
  ...
  // Choose data source (fetched reports override provided reportsList)
  const sourceReports = fetchedReports && fetchedReports.length ? fetchedReports : reportsList;
  ...
  const filteredReports = sourceReports.filter(report => {
    ...
    // Search filter (match against taskTitle if available)
    if (searchQuery.trim() !== '') {
      const query = searchQuery.toLowerCase();
      const hay = (report as any).taskTitle || report.title || '';
      const matchesTitle = String(hay).toLowerCase().includes(query);
  ...
  // Quick stats calculations (use sourceReports so fetchedReports override the initial list)
  const categoryReports = sourceReports.filter(r => r.category === activeSubTab);
    ...
    <div className="space-y-6 max-w-7xl mx-auto font-sans" id="task-reports-container">
      ...
      {/* {isLoading && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[0, 1, 2, 3].map((item) => (
            <div key={item} className="bg-white rounded-3xl border border-[#E2E8F0] shadow-sm p-5 animate-pulse">
  ...
  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-[#FAFBFD] p-5 rounded-2xl border border-[#F1F5F9]">
        {summaries && summaries.length > 0 ? (
          <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center space-x-3.5">
            ...
            <div>
              <span className="text-[10px] text-gray-400 font-bold uppercase block tracking-wider">Assigned</span>
              <span className="text-base font-semibold text-[#0F172A]">{summaries.reduce((acc, s) => acc + (s.total_assigned || 0), 0)} tasks</span>
            </div>
          </div>
        ) : (
          <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center space-x-3.5">
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center space-x-3.5">
          ...
          <div>
            ...
            <span className="text-base font-semibold text-emerald-600">{summaries && summaries.length > 0 ? summaries.reduce((acc, s) => acc + (s.total_submitted || 0), 0) : completedCount} verified</span>
          </div>
        </div>
        ...
        <div className="bg-[#EEF2FF] p-4 rounded-xl border border-blue-100 flex items-center space-x-3.5">
          ...
          <div>
            ...
            <span className="text-base font-bold text-[#2F63FF]">{summaries && summaries.length > 0 ? `${(summaries.reduce((acc, s) => acc + Number(s.avg_score_pct || 0), 0) / summaries.length).toFixed(2)}%` : `${averagePercentage}%`} Accuracy</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-3.5 pt-1">
                    ...
                    >
                      {(report as any).taskTitle || report.title}
                    </h4>
        ...
        <AnimatePresence>
          {expandedCardId && (
            ...
            >
              {(() => {
                const activeReport = sourceReports.find(r => r.id === expandedCardId);
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between border-b border-[#E2E8F0] pb-3">
                      <div>
                        ...
                        <h4 className="text-sm font-bold text-[#0F172A] mt-0.5 leading-tight">{(activeReport as any).taskTitle || activeReport.title}</h4>
                      </div>
                      <button
                    ...
                    <div className="space-y-3 pt-3">
                      ...
                      {/* Submitted media & AI verification summary */}
                      <div className="bg-white border border-dashed border-gray-100 rounded-xl p-4 mt-3">
                        <h6 className="text-[11px] font-bold text-gray-500 mb-2">Submitted Response</h6>
                        <div className="flex items-start gap-4">
                          {activeReport.image_url ? (
                            <img src={activeReport.image_url} alt="submission" className="w-36 h-24 object-cover rounded-md border" />
                          ) : activeReport.video_url ? (
                            <video src={activeReport.video_url} className="w-36 h-24 rounded-md border" controls />
                          ) : activeReport.audio_url ? (
                            <audio src={activeReport.audio_url} controls />
                          ) : (
                            <div className="px-3 py-2 rounded bg-slate-50 text-sm text-gray-600">No media submitted</div>
                          )}

                          <div className="flex-1 text-xs">
                            <h6 className="text-[11px] font-semibold text-gray-600">AI Verification</h6>
                            {activeReport.aiValidation ? (
                              <div className="mt-1 text-[13px] text-gray-700">
                                <div className="flex items-center gap-3">
                                  <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${activeReport.aiValidation.pass ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                                    {activeReport.aiValidation.verdict || (activeReport.aiValidation.pass ? 'PASSED' : 'FAILED')}
                                  </span>
                              ...
                              <div className="text-sm text-gray-500">No AI validation available for this submission.</div>
                            )}
                          </div>
                        </div>
                      </div>
