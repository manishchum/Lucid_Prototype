...
const AI_GATEWAY_URL = process.env.AI_GATEWAY_URL || `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/ai/execute`;
const AI_GATEWAY_INTERNAL_TOKEN = process.env.AI_GATEWAY_INTERNAL_TOKEN || '';
...
async function generateWithRetry(callFn, retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      console.log(`[AI GATEWAY] execute attempt ${attempt}/${retries}`);
      return await callFn();
    } catch (err) {
      lastErr = err;
      console.warn(`[AI GATEWAY] Attempt ${attempt} failed: ${err?.message || err}`);

      if (attempt < retries) {
        const delay = attempt * 4000;
        console.log(`[AI GATEWAY] Waiting ${delay}ms before retry...`);
        await sleep(delay);
      }
    }
  }
...
async function executeAIGateway(payload) {
  ...
  if (!AI_GATEWAY_INTERNAL_TOKEN) {
    throw new Error('AI_GATEWAY_INTERNAL_TOKEN is not configured');
  }
  ...
  try {
    const response = await axios.post(
      ...
      payload,
      {
        headers: {
          ...
          'X-AI-Gateway-Token': AI_GATEWAY_INTERNAL_TOKEN,
        },
        timeout: 180000,
      }
    );
    ...
    console.error('[AI GATEWAY] Backend response:', error.response?.data || error.message);
    throw error;
  }
}
  ...
  for (const mod of modules || []) {
    ...
    if (mod.original_module_id) {
      ...
      const { data: tmData, error: tmError } = await supabase
        ...
        .select('ai_modules, ai_topics, ai_objectives, match_chunks, company_id, uploaded_by')
...
`;
      ...
      // -------------------------------------
      // STEP 2.5: Fetch images and build gateway image payload + URL context
      // -------------------------------------
      const gatewayImages = [];
      ...
      if (matchedChunks && matchedChunks.length > 0) {
        ...
        if (imageError) {
          ...
          console.log(`[AI GATEWAY] Preparing ${prioritizedImages.length} images...`);
          for (const [index, img] of prioritizedImages.entries()) {
            try {
              const path = img.storage_path;
              if (!path) {
                  console.warn(`[AI GATEWAY] Image object is missing storage_path`, img);
                  continue;
              }

              console.log(`[AI GATEWAY] Downloading from private path: ${path}`);
              ...
              if (downloadError) {
                console.error(`[AI GATEWAY] Supabase download error for path ${path}:`, downloadError.message);
                throw downloadError;
              }
              ...
              if (!mimeType || mimeType === 'application/octet-stream') {
                switch (extension) {
                  ...
                  default:
                    console.warn(`[AI GATEWAY] Unknown extension ${extension} for ${path}, using application/octet-stream`);
                    mimeType = 'application/octet-stream';
                }
              }

              console.log(`[AI GATEWAY] Image prep -> file: ${fileName}, ext: ${extension}, blob.type: ${blob.type || 'n/a'}, mimeType: ${mimeType}`);
              ...
              if (signedUrlError) {
                console.warn(`[AI GATEWAY] Failed to create signed URL for ${path}:`, signedUrlError.message);
              ...
              console.log(`[AI GATEWAY] Prepared image ${index + 1}/${prioritizedImages.length}: ${fileName}`);

            } catch (err) {
              console.error(`[AI GATEWAY] Failed to process image ${img.storage_path || img.image_id}:`, err.message);
              if (err?.stack) {
                console.error('[AI GATEWAY] Image error stack:', err.stack);
              }
            }
          }
        }
...
// `;
      ...
      console.log(`[AI GATEWAY] Calling internal execute endpoint...`);
      console.log(`[AI GATEWAY] Module: ${mod.title} (${mod.processed_module_id})`);
      console.log(`[AI GATEWAY] Learning style: ${style}`);
      // console.log(`[AI GATEWAY] Prompt length: ${stylePrompt.length} chars`);

      let response;

      try {
        const aiRequestPayload = {
          feature: 'module_content_generation',
          company_id: String(companyId || ''),
          user_id: String(trainingModuleData?.uploaded_by || ''),
          route: '/worker/generate-module-content',
          prompt_type: 'default',
          variables: {
            ...
            image_url_context: imageUrlContext,
          },
          images: gatewayImages,
          files: [],
          response_format: 'text',
          generation_config: {
            max_output_tokens: ragConfig.rag_max_output_tokens,
            temperature: ragConfig.rag_temperature,
            top_p: ragConfig.rag_top_p,
          },
          metadata: {
            module_id: mod.original_module_id,
            processed_module_id: mod.processed_module_id,
            image_count: gatewayImages.length,
          },
        };

        response = await generateWithRetry(() =>
          executeAIGateway(aiRequestPayload)
        );
      } catch (err) {
        const msg = err?.message || "";
        console.warn("[AI GATEWAY] Full request failed:", msg);
        console.warn("[AI GATEWAY] Error cause:", err?.cause || "No cause available");

        const shouldRetryWithoutImages =
          /fetch failed|sending request|cannot fetch content/i.test(msg);

        if (shouldRetryWithoutImages && gatewayImages.length > 0) {
          console.warn("[AI GATEWAY] Retrying without images after 3 seconds...");
          await sleep(3000);
          ...
          const fallbackPayload = {
            ...
            metadata: {
              ...
              processed_module_id: mod.processed_module_id,
              image_count: 0,
              fallback_without_images: true,
            },
          };

          response = await generateWithRetry(() =>
            executeAIGateway(fallbackPayload)
          );
        } else {
          throw err;
        }
      ...
      if (typeof response?.content === 'string') {
        ...
        aiContent = String(response.content);
      }
      ...
      if (imageUrlReferences.length > 0 && !/<img\b/i.test(aiContent)) {
        ...
        console.log('[AI GATEWAY] Injected fallback figures because model output contained no <img> tags.');
      }
