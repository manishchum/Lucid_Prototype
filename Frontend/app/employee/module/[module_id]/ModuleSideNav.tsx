...
export default function ModuleSideNav({ userId, currentModuleId, sprintModuleId }: ModuleSideNavProps) {
    ...
    // Track the last resolved originalModuleId. When navigating between
    // sub-modules of the same sprint, originalModuleId stays the same so we
    // skip the loading skeleton and silently refresh.
    const lastOriginalModuleIdRef = useRef<string | null>(null);
    ...
    const fetchSprintModules = async () => {
        try {
            ...
            // If sprintModuleId is not provided, resolve it from the DB
            ...
            // Only show the loading skeleton when this is a brand-new sprint.
            // Within the same sprint, the module list doesn't change — just the
            // active highlight — so we update silently to avoid a flicker.
            const isNewSprint = lastOriginalModuleIdRef.current !== originalModuleId;
            if (isNewSprint) {
                setLoading(true);
            }
            ...
            lastOriginalModuleIdRef.current = originalModuleId;
        } catch (error) {
    ...
    const handleModuleClick = (moduleId: string) => {
        // scroll:false avoids the scroll-to-top jump on navigation.
        // Because this component lives in the persistent layout it will NOT
        // remount — only the page content area updates.
        router.push(`/employee/module/${moduleId}`, { scroll: false });
    };
