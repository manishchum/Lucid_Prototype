                       ...
                       {(showAllModules ? assignedModules : assignedModules.slice(0, 3)).map((m) => (
                         <div key={m.id} className="flex flex-col items-start gap-3 p-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:p-4 md:p-6 bg-white">
                           ...
                           <div className="flex gap-2 w-full sm:w-auto flex-shrink-0">
                             ...
                             <button
                               onClick={() => {
                                 const selectedModuleId = String(m.id || "").trim();
                                 if (!selectedModuleId) {
                                   alert("This sprint is not ready yet. Please try again in a moment.");
                                   return;
                                 }
                                 router.push(`/employee/training-plan?module_id=${encodeURIComponent(selectedModuleId)}`);
                               }}
                               className="px-6 sm:px-8 py-2.5 sm:py-3 rounded-xl text-xs sm:text-sm bg-blue-600 text-white font-bold hover:bg-blue-700 flex-1 sm:flex-none h-11 sm:h-12 transition-all duration-200"
                             >
                               Start Your Sprint
                             </button>
                           </div>
