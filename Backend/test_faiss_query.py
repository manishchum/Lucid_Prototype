...
# def build_rag_prompt(user_query: str, retrieved: List[Dict]) -> str:
    context_blocks = []
    for r in retrieved:
        context_blocks.append(f"Context {r['rank']} (score: {r['score']:.4f}):\n{r['text']}")
...
# **Module Context:**
# 1. The title, topics, and learning objectives provided in the user query define the scope and focus of the training module.
# 2. The top-k chunks contain critical company-specific and domain-specific information that must be integrated into the content.

# **New Critical Requirement for Content Accuracy:**
# 1. You will strictly use only the information provided in the user query and the supplied top-k chunks. You must not infer, assume, or add any information that is not explicitly present in these sources.
# 2. You must not hallucinate, guess, or invent any company names, products, policies, regulations, KPIs, numbers, tools, or examples.
# 3. All proper names, company names, medicine names, chemical names, technical terms, numeric values, thresholds, caps, dates, workflows, products, tools, or regulations present in the query or chunks must appear exactly as provided. Do not replace, modify, or invent anything.
# 4. If the chunks or query do not provide sufficient detail, remain abstract and conceptual. Do not fabricate examples or specifics.
# 5. Do not introduce any information, tools, vendors, or examples that are not explicitly present in the topics or chunks. All content must remain strictly within the scope of the provided context.
...
# Expansion is allowed ONLY within the scope of the provided topics and objectives and information passed for context.
...
### ROLE
    return f"""
You are an expert Instructional Designer and Technical Writer. Your task is to create a high-quality, professional e-learning module based EXCLUSIVELY on the provided source materials.

### SOURCE DATA (PRIORITIZED)
The following information is retrieved from the organization's knowledge base. 
[IMPORTANT]: Information in 'PRIMARY SOURCE' should be treated as the foundational truth. Supporting sources provide additional depth.

{context}

### INSTRUCTIONS
1. **Scope:** Use the following Query to define the title, topics, and objectives of the module:
   {user_query}

2. **Grounding (Strict):** You must ONLY use facts, names, KPIs, and processes found in the SOURCE DATA. 
   - If the source mentions a specific company, product, or regulation, use it verbatim.
   - If the source is missing specific details for a topic, explain the concept theoretically without inventing fake names or data.
   - DO NOT introduce outside tools, vendors, or "best practices" not mentioned in the text.

3. **Tone & Style:** Professional, educational, and structured. 
4. **Output Format:** Provide ONLY valid HTML5. Use semantic tags (<h2>, <h3>, <table>, <ul>, <blockquote>). Do not use Markdown characters.

### MODULE STRUCTURE
- **Learning Objectives:** Listed as an <ol>.
- **Sections:** Descriptive headings with thorough explanations.
- **Comparison/Data Tables:** Use <table> for any lists, steps, or KPIs found in the text.
- **Key Takeaways:** Highlighted using <blockquote>.
- **Learning Activity:** A practical exercise or reflection based on the module content.
- **Summary:** A bulleted <ul> list of core points.

### FINAL MANDATE
Do not hallucinate. Do not reference "the provided text" or "Source 1" in the final output; write it as a finished training module for the learner.'''
""".strip()
# =========================
# GEMINI CALL
# =========================
def build_rag_prompt(user_query: str, retrieved: List[Dict]) -> str:
    ...
    return f"""
### ROLE
You are a Precise Technical Instructor. Your goal is to generate a training module in a structured, point-by-point format.
...
4. **Point-Based Output:** Present information using concise bullet points under <h2> and <h3> headers.
...
- Produce the output in valid HTML5 points (<ul>, <li>).
...
if __name__ == "__main__":
    ...
    # 🔹 YOUR TEST QUERY
    QUERY = """
    "title":"title":"Artificial Intelligence Defined",
    "topics":["Human Abilities vs. AI Equivalents (Computer Vision, NLP, Forecasting, Automation",
        "History of AI (Turing Test, McCarthy, General Problem Solver",
        "Limits of Computer Understanding (Rules and Pattern Matching",
        "The Double Materiality" approach to sustainability"
        ]
    "objectives":["Learners will define Artificial Intelligence as a set of algorithms designed to perform tasks requiring human intelligence",
    "Learners will map human abilities like seeing and speaking to their specific AI equivalents.",
    "Learners will identify the historical milestones and the transition from mathematical formulas to early intelligent systems."
    ]
    """
