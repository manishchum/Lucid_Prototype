...
async def create_scenario( 
   ...
   try: 
       ...
       payload = { 
           ...
           "aiObjective": [request_data.aiObjectives] if request_data.aiObjectives 
else None, 
           "endConditions": [request_data.endConditions] if 
request_data.endConditions else None, 
           "maxDuration": [request_data.maxDuration] if request_data.maxDuration 
else None, 
           ...
           "passingScore": [request_data.cutoffScore] if request_data.cutoffScore 
else None, 
       ...
       if result.data: 
           return { 
               "success": True, 
               "data": result.data[0] if isinstance(result.data, list) else 
result.data, 
       ...
       raise HTTPException(status_code=500, detail=f"Error creating scenario: 
{str(e)}") 
...
async def update_scenario( 
   ...
   try: 
       ...
       # Verify ownership 
       existing = 
supabase.table("scenarios").select("scenario_id").eq("scenario_id", 
scenario_id).eq("company_id", company_id).execute() 
       
       if not existing.data: 
           raise HTTPException(status_code=404, detail="Scenario not found or 
access denied") 
       ...
       # Update in Supabase 
       result = supabase.table("scenarios").update(payload).eq("scenario_id", 
scenario_id).execute() 
...
result.data, 
       ...
       raise HTTPException(status_code=500, detail=f"Error updating scenario: 
{str(e)}") 
...
access denied") 
       ...
       result = supabase.table("scenarios").delete().eq("scenario_id", 
scenario_id).execute() 
       ...
       raise HTTPException(status_code=500, detail=f"Error deleting scenario: 
{str(e)}") 
...
async def fetch_user_data( 
   ...
   try: 
       ...
       result = supabase.table("users").select("user_id, company_id").eq("email", 
user_email).execute() 
       ...
       raise HTTPException(status_code=500, detail=f"Error fetching user data: 
{str(e)}") 
...
async def list_scenarios( 
   ...
   """ 
   List all custom roleplay scenarios for the company 
   """ 
   try: 
       company_id = effective_company_id 
       result = supabase.table("scenarios").select("*").eq("company_id", 
company_id).execute() 
       return { 
           "success": True, 
       ...
       raise HTTPException(status_code=500, detail=f"Error fetching scenarios: 
{str(e)}") 
...
async def get_scenario( 
   ...
   """ 
   Get a specific custom roleplay scenario 
   """ 
   try: 
       company_id = effective_company_id 
       result = supabase.table("scenarios").select("*").eq("scenario_id", 
scenario_id).eq("company_id", company_id).execute() 
