...
async def processAndStoreResults(moduleId: str, message: str, sprint_insights: list = None):
    ...
    if sprint_insights is not None:
        update_payload["sprint_insights"] = sprint_insights
    ...
    if sprint_insights is not None:
        upsert_payload["sprint_insights"] = sprint_insights
...
async def processTextContent(
    ...
    sprint_insights = []
    try:
        insights_response = await AI.execute(
            AIRequest(
                feature="module_chat",
                company_id=str(companyId or ""),
                user_id=str(userId or ""),
                route="/openai-upload/insights",
                prompt_type="default",
                variables={
                    "moduleTitle": "Sprint Document",
                    "moduleContent": text[:10000], # Keep it within limits
                    "conversationContext": "",
                    "userMessage": "Based on the provided document, generate 5 basic overview questions. The FIRST question MUST be exactly 'What are the 5 key insights from this document?'. The other 4 questions should be generated by you based on the full document context (e.g., questions asking for a summary or overarching themes). Format the output strictly as a JSON array of objects with 'question' and 'answer' keys. Do not include markdown formatting like ```json.",
                },
                response_format="json",
            )
        )
        insights_content = str(insights_response.content or "").strip()
        if insights_content.startswith("```json"):
            insights_content = insights_content[7:]
        if insights_content.endswith("```"):
            insights_content = insights_content[:-3]
        sprint_insights = json.loads(insights_content.strip())
    except Exception as e:
        print(f"[processTextContent] Failed to generate sprint insights: {e}")
        ...
        try:
            insights_response = await AI.execute(
                AIRequest(
                    ...
                    variables={
                        "moduleTitle": "Sprint Document",
                        "moduleContent": document_content[:10000], # Keep it within limits
                        "conversationContext": "",
                        "userMessage": "Based on the provided document, generate 5 basic overview questions. The FIRST question MUST be exactly 'What are the 5 key insights from this document?'. The other 4 questions should be generated by you based on the full document context (e.g., questions asking for a summary or overarching themes). Format the output strictly as a JSON array of objects with 'question' and 'answer' keys. Do not include markdown formatting like ```json.",
                    },
                    response_format="json",
            ...
            print(f"[openai_upload_file] Failed to generate sprint insights: {e}")


        return await processAndStoreResults(moduleId, message.strip(), sprint_insights=sprint_insights)
