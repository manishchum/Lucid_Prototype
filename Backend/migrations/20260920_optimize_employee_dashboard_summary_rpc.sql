-- Migration: High-Scale Optimization for Employee Dashboard Summary RPC and Company Data Separation
-- Date: 2026-09-20

-- 1. Ensure Composite Covering Indexes Exist
...
-- 2. Optimized Full Dashboard RPC (Using JOIN instead of nested IN subquery)
...
-- 2. Separate Company Static Data RPC (Cacheable per company in Redis)
CREATE OR REPLACE FUNCTION get_company_static_dashboard_data(
    p_company_id UUID
)
RETURNS JSONB
LANGUAGE plpgsql
